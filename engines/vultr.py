...
def copy_file_to_node(localPath, nodeConfig, remotePath):
	return run_cmd(["scp", "-r", f"-i./keys/vultr/{nodeConfig['key']}", localPath, f"{nodeConfig['user']}@{nodeConfig['ip']}:{remotePath}"])

# f"scp://{nodeConfig['user']}@{nodeConfig['ip']}/{remotePath}"
def copy_file_from_node(remotePath, nodeConfig, localPath):
	return run_cmd(["scp", "-r", f"-i./keys/vultr/{nodeConfig['key']}", f"{nodeConfig['user']}@{nodeConfig['ip']}:{remotePath}", localPath])
...
def make_announcement(node, announcementList):
	...
	templateFile = open(f"./config/vultr/templates/{nodeConfig['template']}")
	template = templateFile.read()
	templateFile.close()

	templateFile4 = open(f"./config/vultr/templates/{nodeConfig['template4']}")
	...
	tmpConfigPath = f"./config/vultr/templates/{nodeConfig['template']}.{int(random.random() * 1000000)}.tmp"
	...
	tmpConfigPath4 = f"./config/vultr/templates/{nodeConfig['template4']}.{int(random.random() * 1000000)}.tmp"
